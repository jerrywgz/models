...
def preprocess(samples, 
    ...
    if mode == 'train':
        for batch_position in range(len(samples)):
            ...
            if bboxes == []:
                bboxes.append([0., 0., img.size[1], img.size[0]])
            sample_distorted_bounding_box = utils.generate_samples_imagenet(
                           img.size[1], img.size[0], sampler, bboxes)
            bbox_begin, bbox_size, distort_bbox = utils.sample_distorted_bounding_box
...
def train(batch_size, data_dir=DATA_DIR):
    file_list = os.path.join(data_dir, 'train_with_bbox.txt')
    return _reader_creator(file_list, 'train', batch_size, 
                           shuffle=True, color_jitter=True, 
                           rotate=False, data_dir=data_dir)


def val(batch_size, data_dir=DATA_DIR):
    file_list = os.path.join(data_dir, 'val.txt')
    return _reader_creator(file_list, 'val', batch_size,
                           shuffle=False, data_dir=data_dir)


def test(batch_size, data_dir=DATA_DIR):
    file_list = os.path.join(data_dir, 'val.txt')
    return _reader_creator(file_list, 'test', batch_size,
                           shuffle=False, data_dir=data_dir)
