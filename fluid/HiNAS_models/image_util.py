...
class sampler():
    def __init__(self, max_trial, min_scale, max_scale,
                 min_aspect_ratio, max_aspect_ratio, min_overlap):
...
def satisfy_sample_constraint(sampler, sample_bbox, bboxes):
    ...
    for i in range(len(bboxes)):
        object_bbox = BBox(bboxes[i][1], bboxes[i][2],
                           bboxes[i][3], bboxes[i][4])
        object_area = bbox_area(object_box)
...
def generate_samples_imagenet(origin_w, origin_h, sampler, bboxes):
    ...
    bndbox = (float(sample_bbox.xmin), float(sample_bbox.ymin), \
              float(sample_bbox.xmax), float(sample_bbox.ymax))
...
def random_hsv_in_yiq(image, lower_saturation, upper_saturation,
                      max_delta_hue=0, scale_value=1):
...
def distort_color(image, batch_position=0):
    def distort_fn_0(image=image):     
        ...
        image = random_hsv_in_yiq(image, lower_saturation=0.5, 
                upper_saturation=1.5, max_delta_hue=0.2 * math.pi)
